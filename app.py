    ...
    with st.form("tabulation_form", clear_on_submit=True):
        ...
        with col2:
            ...
            if social_network == "PlayStore":
                rating = st.feedback("stars") + 1 # Streamlit feedback returns 0-4, we want 1-5
...
with tabs[1]:
    ...
    if not df_filtered.empty:
        ...
        # Métrica extra para PlayStore
        df_play = df_filtered[df_filtered['social_network'] == 'PlayStore']
        if not df_play.empty:
            avg_rating = df_play['rating'].mean()
            st.metric("Avaliação Média (PlayStore)", f"{avg_rating:.1f} ⭐")
...
with tabs[2]:
    ...
    if not df_filtered.empty:
        for _, row in df_filtered.iterrows():
            ...
            stars_display = f" {'⭐' * int(row['rating'])}" if row['social_network'] == 'PlayStore' and row['rating'] > 0 else ""
            st.markdown(f"""
                <div class="card {network_class}">
                    <div style="display: flex; justify-content: space-between;">
                        <strong>{row['social_network']} | {row['client_name']}{stars_display}</strong>
